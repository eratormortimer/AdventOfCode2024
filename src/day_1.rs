...
pub fn get_distance(list_1:Vec<String>, list_2:Vec<String>) -> Option<i32> {
...
pub fn get_similarity_score(list_1:Vec<String>, list_2:Vec<String>) -> Option<i32> {
    // Convert strings to integers
    let list_1: Vec<i32> = list_1
    .into_iter()
    .map(|s| s.parse::<i32>().unwrap_or(0)) // Parse each string to i32, defaulting to 0 if it fails
    .collect();
    let list_2: Vec<i32> = list_2
    .into_iter()
    .map(|s| s.parse::<i32>().unwrap_or(0)) // Parse each string to i32, defaulting to 0 if it fails
    .collect();
